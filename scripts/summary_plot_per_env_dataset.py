...
if __name__ == '__main__':
    ...
    for run in api.runs('koulanurag/cque-baselines-final-uncertainty-test'):
                        # filters={'state': 'finished', 'config.env_name':'d4rl:maze2d-large-v1',
                        #          'config.dataset_name':'1k'}):
        print(run)
    ...
    for uncertainty_test_type in ['ensemble-voting', 'unpaired-confidence-interval','paired-confidence-interval' ]:
        for env in envs.tolist():
            dataset_names = np.unique(ensemble_data_df[(ensemble_data_df['uncertainty_test_type']
            ...
            for dataset_name in dataset_names:
                ...
                uncertainty_plots = {
                    _name: {'dynamics_type': plt.subplots(1, sub_plot_count, figsize=(10, 2.5), squeeze=False,
                            ...
                            'constant_prior': plt.subplots(1, sub_plot_count, figsize=(10, 2.5), squeeze=False,
                                                           layout='constrained'),
                            'ensemble_mixture': plt.subplots(1, sub_plot_count, figsize=(10, 2.5), squeeze=False,
                                                             layout='constrained')}
                ...
                ensemble_mixtures = sorted(np.unique(ensemble_type_data['ensemble_mixture']))
                ...
                for entity_name, entity_itr, type_data in \
                        [
                         ...
                         ('ensemble_mixture', ensemble_mixtures, ensemble_type_data)]:
                ...
                uncertainty_plots['accuracy_vs_abstain']['constant_prior'][1][0, 0].legend(
                    labels=['constant_prior' if x is True else 'without constant prior' for x in
                            deterministic_dynamics]).set_visible(False)
                uncertainty_plots['accuracy_vs_abstain']['ensemble_mixture'][1][0, 0].legend(
                    labels=['use ensemble-mixture' if x is True else 'no ensemble mixture' for x in
                            deterministic_dynamics]).set_visible(False)

                for _name in uncertainty_plots:
                    _path = os.path.join(os.getcwd(), 'uncertainty_test_plots', env, uncertainty_test_type, dataset_name, _name)
                    ...
                    for (_fig, _axs), _name, _fig_title, ncol, bbox_to_anchor in [
                        ...
                        (uncertainty_plots[_name]['ensemble_mixture'], 'ensemble_mixture',
                         "Comparison between rollout variations", 1, (0.9, -0.1)),
