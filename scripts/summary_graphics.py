...
def score(metric_info):
    cf = round(abs(metric_info["confidence_interval"][0] - metric_info["mean"]), 3)
    return (
        "$"
        + (
            str(round(metric_info["mean"], 3))
            + "\pm"
...
def metrics(metrics_data, sr_coverage_data, key):
    ...
    for _num in np.unique(metrics_data[key].values):
        env_names = np.unique(
            metrics_data[metrics_data[key] == _num]["env_name"].values
        )
        for env_name in env_names:
            key_metric_data = metrics_data[
                (metrics_data[key] == _num) & (metrics_data["env_name"] == env_name)
            ]
            ...
            risk = []
            coverage = []
            key_src_data = sr_coverage_data[
                (sr_coverage_data[key] == _num)
                & (sr_coverage_data["env_name"] == env_name)
            ]
            for tau in key_src_data["taus"].unique():
                risk.append(
                    key_src_data["risk"][key_src_data["taus"] == tau].values.tolist()
                )
                coverage.append(
                    key_src_data["coverage"][
                        key_src_data["taus"] == tau
                    ].values.tolist()
                )
            ...
            info[env_name][_num] = {
                "aurcc": {
                    ...
                    "confidence_interval": stats.norm.interval(
                        0.95, aurcc.mean(), aurcc.std() / np.sqrt(n) + 1e-5
                    ),
                    "n": len(aurcc),
                },
                "rpp": {
                    ...
                    "std": rpp.std(),
                    "confidence_interval": stats.norm.interval(
                        0.95, rpp.mean(), rpp.std() / np.sqrt(n) + 1e-5
                    ),
                    "n": len(rpp),
                },
                "cr_10": {
                    ...
                    "std": cr_10.std(),
                    "confidence_interval": stats.norm.interval(
                        0.95, cr_10.mean(), cr_10.std() / np.sqrt(n) + 1e-5
                    ),
                    "n": len(cr_10),
                },
                "loss": {
                    ...
                    "std": loss.std(),
                    "confidence_interval": stats.norm.interval(
                        0.95, loss.mean(), loss.std() / np.sqrt(n) + 1e-5
                    ),
                    "n": len(loss),
                },
                "risk": {
                    "mean": np.mean(risk, axis=1),
                    "std": np.std(risk, axis=1),
                    "n": len(risk),
                },
                "coverage": {
                    "mean": np.mean(coverage, axis=1),
                    "std": np.std(coverage, axis=1),
                    "n": len(coverage),
                },
            }
    return info
...
def sr_coverage_plot_and_tex(info_dict, category_name, plot_name, path):
    ...
    for env_i, env_name in enumerate(info_dict.keys()):
        ...
        for cat in info_dict[env_name]:
            ...
            axs[0, env_i].plot(
                x, y, color=color_dict[cat], label=prettify_category_name(cat)
            )
    ...
    tex += "\centering\n"
    tex += (
        "\includegraphics[scale=0.7]{assets/query-eval/"
        + category_name
        + "/"
        + plot_name
        + "/sr_coverage.png}\n"
    ...
    tex += (
        "\caption{Selective-risk coverage curves for  \emph{"
        + plot_name
        + "}"
        + " in \emph{"
        + category_name
...
def latex_table(info_dict, category_name, table_name, path):
    tex = (
        "\\begin{table}[h!]"
        + "\n"
        + "\caption{Evaluation metrics for  \emph{"
        + table_name
        + "}"
        + " comparison in \emph{"
        + category_name
    ...
    tex += (
        "Env. & " + table_name + " & AURCC$(\\downarrow)$ &"
        " RPP$(\\downarrow)$ &"
        " $CR_K(\\uparrow)$ & "
        " loss$(\\downarrow)$ \\\\" + "\n"
    )
    ...
    for env_i, env_name in enumerate(info_dict.keys()):
        if len(info_dict[env_name]) > 1:
            tex += (
                "\multirow{"
                + str(len(info_dict[env_name]))
                + "}{3.6em}{"
                + str(prettify_env_name(env_name))
                + "}  "
            )
        else:
...
def generate_graphics(category_name, data_df, sr_coverage_df):
    ...
    ensemble_count_info = metrics(ensemble_data, ensemble_sr_data, "ensemble_count")
    latex_table(
        ensemble_count_info,
        category_name,
        "ensemble-count",
    ...
    dataset_quality_info = metrics(dataset_data, dataset_sr_data, "dataset_name")
    latex_table(
    ...
    latex_table(
        horizon_info, category_name, "horizon", os.path.join(root_dir, "horizon")
    )
    sr_coverage_plot_and_tex(
        horizon_info, category_name, "horizon", os.path.join(root_dir, "horizon")
    )
    ...
    latex_table(
        prior_info, category_name, "prior-scale", os.path.join(root_dir, "prior-scale")
    )
    sr_coverage_plot_and_tex(
        prior_info, category_name, "prior-scale", os.path.join(root_dir, "prior-scale")
    )
    ...
    latex_table(
        mixture_info, category_name, "mixture", os.path.join(root_dir, "mixture")
    )
    sr_coverage_plot_and_tex(
        mixture_info, category_name, "mixture", os.path.join(root_dir, "mixture")
    )
    ...
    latex_table(
        norm_info, category_name, "normalize", os.path.join(root_dir, "normalization")
    )
    sr_coverage_plot_and_tex(
        norm_info, category_name, "normalize", os.path.join(root_dir, "normalization")
    )
...
def main():
    ...
    run_i = 0
    for run in api.runs(
        "koulanurag/opcc-baselines-uncertainty-test", filters={"state": "finished"}
    ):
        print(run_i, run)
        run_i += 1

        # restore evaluation metrics
        table_file = wandb.restore(
            run.summary.get("eval-metrics").get("path"), run_path="/".join(run.path)
        )
        ...
        _coverage_dict = pickle.load(
            open(
                wandb.restore(
                    "sr_coverage_dict.pkl", "/".join(run.path), replace=True
                ).name,
                "rb",
            )
        )
                    ...
                    pd.DataFrame.from_dict(
                        {
                            ...
                            **{
                                "ensemble_count": [
                                    count
                                    for _ in range(len(count_horizon_info["taus"]))
                                ],
                                "horizon": [
                                    horizon
                                    for _ in range(len(count_horizon_info["taus"]))
        ...
        for _df in [query_df, _coverage_df]:
            ...
            # dynamics factors
            _df["dynamics_type"] = [
                run.config["dynamics_type"] for _ in range(len(_df))
            ]
            _df["deterministic"] = [
                run.config["deterministic"] for _ in range(len(_df))
            ]
            _df["constant_prior_scale"] = [
                run.config["constant_prior_scale"] for _ in range(len(_df))
            ]
            _df["dynamics_seed"] = [
                run.config["dynamics_seed"] for _ in range(len(_df))
            ]
            _df["normalize"] = [run.config["normalize"] for _ in range(len(_df))]

            # evaluation factors
            _df["mixture"] = [run.config["mixture"] for _ in range(len(_df))]
            _df["clip_obs"] = [run.config["clip_obs"] for _ in range(len(_df))]
            _df["clip_reward"] = [run.config["clip_reward"] for _ in range(len(_df))]

            # uncertainty factors
            _df["uncertainty_type"] = [
                run.config["uncertainty_test_type"] for _ in range(len(_df))
            ]
    ...
    for category_name, env_names in [
        ("maze", maze2d_envs),
        ("gym-mujoco", gym_mujoco_envs),
    ]:
        generate_graphics(
            category_name,
            eval_metrics_df[eval_metrics_df["env_name"].isin(env_names)],
            sr_coverage_df[sr_coverage_df["env_name"].isin(env_names)],
        )
