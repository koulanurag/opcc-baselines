...
@pytest.mark.parametrize(
    "obs_min, obs_max, reward_min, reward_max,  "
...
def test_clip(
    obs_min, obs_max, reward_min, reward_max, dynamics_type, deterministic, prior_scale
):
    import gym
    ...
    network = EnsembleDynamicsNetwork(
        env_name=env_name,
    ...
    _obs_max = np.ones((network.num_ensemble, env.observation_space.shape[0])) * obs_max
    _obs_min = np.ones((network.num_ensemble, env.observation_space.shape[0])) * obs_min
    ...
    for run in range(3):  # do multiple runs
        ...
        for step_i in range(1, horizon + 1):
            ...
            step_obs, reward, done = network.step(step_obs.float(), step_action)

            assert (reward_min <= reward).all() and (
                reward <= reward_max
            ).all(), " reward out of bounds"
            assert (obs_min <= step_obs).all() and (
                step_obs <= obs_max
            ).all(), " obs. out of bounds"
