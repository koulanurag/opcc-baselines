...
def test_ensemble_voting():
    ...
    # #########################################################
    pred_a = np.repeat(np.expand_dims(np.array(return_a), 1), num_ensemble, axis=1)
    pred_b = np.repeat(np.expand_dims(np.array(return_b), 1), num_ensemble, axis=1)
    ...
    uncertainty_dict = ensemble_voting(eval_df, ensemble_size_interval, num_ensemble)
...
def test_confidence_interval(paired):
    ...
    uncertainty_dict = confidence_interval(
        eval_df, ensemble_size_interval, num_ensemble, paired=paired
    )
