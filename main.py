...
def get_args(arg_str: str = None):
    ...
    job_args.add_argument("--no-cuda", action="store_true", help="no cuda usage")
    job_args.add_argument(
        "--job",
        required=True,
        choices=["train-dynamics", "evaluate-queries", "uncertainty-test"],
    )
    # paths
    ...
    wandb_args.add_argument(
        "--use-wandb", action="store_true", help="use Weight and bias visualization lib"
    )
    wandb_args.add_argument(
        "--wandb-dir",
    ...
    dynamics_resume_args = parser.add_argument_group(
        "args for resuming " "dynamics training"
    )
    dynamics_resume_args.add_argument(
        "--resume", choices=["local", "wandb"], help="resumes training"
    )
    dynamics_resume_args.add_argument(
        "--wandb-run-path",
    ...
    dynamics_args.add_argument(
        "--env-name", default="HalfCheetah-v2", help="name of the environment"
    )
    dynamics_args.add_argument(
        "--dataset-name", default="random", help="name of the dataset"
    )
    dynamics_args.add_argument(
        "--dynamics-type",
    ...
    dynamics_args.add_argument(
        "--deterministic",
        action="store_true",
        help="if True, we use deterministic model " "otherwise stochastic",
    )
    dynamics_args.add_argument(
        "--dynamics-seed", default=0, type=int, help="seed for training dynamics "
    )
    dynamics_args.add_argument(
        "--log-interval", default=1, type=int, help="log interval for training dynamics"
    )
    dynamics_args.add_argument(
        "--dynamics-checkpoint-interval",
        type=int,
        default=1,
        help="update interval to save" " dynamics checkpoint ",
    )
    dynamics_args.add_argument(
        "--hidden-size", type=int, default=200, help="hidden size for Linear Layers "
    )

    dynamics_args.add_argument(
        "--update-count", type=int, default=100, help="epochs for training "
    )
    dynamics_args.add_argument(
        "--dynamics-batch-size",
        type=int,
    ...
    dynamics_args.add_argument(
        "--observation-loss-coeff",
        type=int,
        default=1,
        help="obs. loss coefficient for training ",
    )
    dynamics_args.add_argument(
        "--grad-clip-norm", type=float, default=5.0, help="gradient clipping norm"
    )
    dynamics_args.add_argument(
        "--dynamics-lr", type=float, default=1e-3, help="learning rate for Dynamics"
    )
    dynamics_args.add_argument(
        "--normalize",
        action="store_true",
        help="normalizes the network inputs" " with 0 mean and 1 std",
    )
    dynamics_args.add_argument(
        "--num-ensemble", default=1, type=int, help="number of dynamics for ensemble "
    )
    dynamics_args.add_argument(
    ...
    queries_args.add_argument(
        "--restore-dynamics-from-wandb",
        action="store_true",
        help="restore model from wandb run",
    )
    queries_args.add_argument(
        "--wandb-dynamics-run-path", type=str, help="wandb run id if restoring model"
    )
    queries_args.add_argument(
        "--mixture",
        action="store_true",
    ...
    queries_args.add_argument(
        "--eval-runs", type=int, default=1, help="run count for each query evaluation"
    )
    queries_args.add_argument(
        "--eval-batch-size",
        type=int,
    ...
    queries_args.add_argument(
        "--clip-obs",
        action="store_true",
        help="clip the observation space with bounds for" " query evaluation",
    )
    queries_args.add_argument(
        "--clip-reward",
        action="store_true",
        help="clip the reward with dataset bounds" " for query evaluation",
    )

    # uncertainty-test arguments
    uncertain_args = parser.add_argument_group("args for uncertainty-test")
    uncertain_args.add_argument(
    ...
    uncertain_args.add_argument(
        "--restore-query-eval-data-from-wandb",
        action="store_true",
        help="get query evaluation data from wandb",
    )
    uncertain_args.add_argument(
        "--wandb-query-eval-data-run-path",
        type=str,
...
def _evaluate_queries(args, job_args, dynamics_args, queries_args, verbose=False):
    # set-up config
    if args.restore_dynamics_from_wandb:
        # get remote config from wandb
        assert (
            args.wandb_dynamics_run_path is not None
        ...
        wandb.restore(
            name=name, run_path=args.wandb_dynamics_run_path, replace=True, root=root
        )
    else:
        config = BaseConfig(args, dynamics_args)
    ...
    if args.use_wandb:
        ...
        wandb.config.update(
            {x.dest: vars(args)[x.dest] for x in job_args._group_actions}
        )
        wandb.config.update(
            {x.dest: vars(args)[x.dest] for x in dynamics_args._group_actions}
        )
        wandb.config.update(
            {x.dest: vars(args)[x.dest] for x in queries_args._group_actions}
        )
    ...
    if args.use_wandb:
        ...
        wandb.log(
            {
                "mean/q-value-comparison-a": scatter(
                    table, x="pred_a_mean", y="return_a", title="q-value-comparison-a"
                )
            }
        )
        wandb.log(
            {
                "mean/q-value-comparison-b": scatter(
                    table, x="pred_b_mean", y="return_b", title="q-value-comparison-b"
                )
            }
        )
...
def _train_dynamics(args, job_args, dynamics_args):
    ...
    if args.resume is not None:
        if args.resume == "wandb":
            ...
            os.makedirs(root, exist_ok=True)
            wandb.restore(
                name=name, run_path=args.wandb_run_path, replace=True, root=root
            )
    ...
    if args.use_wandb:
        wandb.init(
            ...
            settings=wandb.Settings(start_method="thread"),
            id=(
                args.wandb_run_path.split("/")[-1] if args.resume is not None else None
            ),
...
def reverse_pair_proportion(loss, confidences):
    ...
    rpp = np.logical_and(
        np.expand_dims(loss, 1) < np.expand_dims(loss, 1).transpose(),
        np.expand_dims(confidences, 1) < np.expand_dims(confidences, 1).transpose(),
    ).mean()
    return rpp
...
def _uncertainty_test(args, job_args, dynamics_args, queries_args, uncertainty_args):
    # restore query evaluation data
    if args.restore_query_eval_data_from_wandb:
        assert (
            args.wandb_query_eval_data_run_path is not None
        ...
        table_file = wandb.restore(table_file_path, args.wandb_query_eval_data_run_path)
    ...
    if args.use_wandb:
        ...
        wandb.config.update(
            {x.dest: vars(args)[x.dest] for x in uncertainty_args._group_actions}
        )
    ...
    for ensemble_count in sorted(uncertainty_dict.keys()):
        ...
        for horizon in horizons + [None]:
            if horizon is None:  # get data for all horizons
                ...
                for _h in horizons:
                    for __k in uncertainty_dict[ensemble_count][_h]:
                        v[__k] += list(uncertainty_dict[ensemble_count][_h][__k])
            ...
            # evaluation metrics
            selective_risks, coverages, taus = sr_coverages(loss, v["confidence"])
            ...
            eval_metric_df.append(pd.DataFrame({_k: [_v] for _k, _v in _log.items()}))
    ...
    if args.use_wandb:
        ...
        for ensemble_count in sr_coverages_data:
            for horizon, _data in sr_coverages_data[ensemble_count].items():
                data = [[x, y] for (x, y) in zip(_data["coverage"], _data["risk"])]
                table = wandb.Table(data=data, columns=["coverage", "risk"])

                title = "SR-Coverage Curve (Ensemble: {}, Horizon:{})".format(
                    ensemble_count, horizon
                )
                wandb.log(
                    {
                        "selective_risk-coverage-ensemble-count-{}"
                        "-horizon-{}".format(ensemble_count, horizon): wandb.plot.line(
                            table, "coverage", "risk", title=title
                        )
                    }
                )
...
def main():
    ...
    try:
        if args.job == "train-dynamics":
            ...
            _evaluate_queries(args, job_args, dynamics_args, queries_args, verbose=True)
        elif args.job == "uncertainty-test":
            _uncertainty_test(
                args, job_args, dynamics_args, queries_args, uncertainty_args
            )
        else:
