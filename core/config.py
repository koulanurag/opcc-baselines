...
class BaseConfig(object):
    ...
    def __init__(self, args, dynamics_args):
        ...
        sorted_dyn_args = sorted(dynamics_args._group_actions, key=lambda x: x.dest)
    ...
    def get_uniform_dynamics_network(self) -> EnsembleDynamicsNetwork:
        return EnsembleDynamicsNetwork(
    ...
    def evaluate_queries_path(self, args, queries_args) -> os.path:
        ...
        sorted_args = sorted(queries_args._group_actions, key=lambda x: x.dest)
    ...
    def uncertainty_exp_dir(self, args, queries_args, uncertainty_args) -> os.path:
        # hyper-parameters hash for experiment saving
        sorted_args = sorted(queries_args._group_actions, key=lambda x: x.dest)
        query_args_str = [str(vars(args)[hp.dest]) for hp in sorted_args]
        query_args_byte = bytes("".join(query_args_str), "ascii")
        ...
        # hyper-parameters hash for experiment saving
        sorted_args = sorted(uncertainty_args._group_actions, key=lambda x: x.dest)
        ...
        _dir = os.path.join(self.exp_dir_path, query_args_hash, uncertainty_args_hash)
        os.makedirs(_dir, exist_ok=True)
