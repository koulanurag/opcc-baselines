...
def ensemble_voting(
    eval_df: pd.DataFrame, ensemble_size_interval: int, num_ensemble: int
):
    ...
    for ensemble_count in np.arange(
        ...
        for horizon in horizon_candidates:
            ...
            prediction, confidence = _voting(
                pred_a[:, :ensemble_count][_filter], pred_b[:, :ensemble_count][_filter]
            )
            info = {
