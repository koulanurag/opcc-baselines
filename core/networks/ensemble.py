...
class EnsembleDynamicsNetwork:
    ...
    def update(
        self, replay_buffers, update_count: int, batch_size: int
        ...
        for i in range(self.num_ensemble):
            ...
            for batch_i in range(update_count):
                ...
                _loss = dynamics.update(
                    batch.obs, batch.action, batch.next_obs, batch.reward.unsqueeze(-1)
                )
    ...
    def to(self, device, *args, **kwargs):
        for i in range(self.num_ensemble):
            ...
            setattr(
                self, ensemble_i, getattr(self, ensemble_i).to(device, *args, **kwargs)
            )
        return self
