...
def evaluate_queries(
    ...
    for (policy_a_id, policy_b_id), query_batch in tqdm(
        queries.items(), desc="Policy Pairs"
    ):
        # get policies
        ...
        for horizon in tqdm(np.unique(horizons), desc="Horizons", disable=verbose):
            _filter = horizons == horizon
            pred_a[_filter, :] = mc_return(
                network=network,
...
def mc_return(
    ...
    for batch_idx in tqdm(
        range(0, returns.shape[0], eval_batch_size), desc="Batch", disable=verbose
    ):
