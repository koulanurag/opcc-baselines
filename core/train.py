...
def train_dynamics(config: BaseConfig):
    ...
    dataset = opcc.get_qlearning_dataset(config.args.env_name, config.args.dataset_name)
    ...
    if config.args.normalize:
        ...
        dataset["next_observations"] = (
            dataset["next_observations"] - obs_mean
        ) / obs_std
    ...
    for ensemble_i in tqdm(range(config.args.num_ensemble), desc="bootstrap sampling"):
        # bootstrap sampling
    ...
    for update_i in range(
        start_count, config.args.update_count + 1, config.args.log_interval
    ):
        ...
        # save
        torch.save(
            {"network": network.state_dict(), "update": update_i},
